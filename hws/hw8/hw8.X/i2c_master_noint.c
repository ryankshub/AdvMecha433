...
void i2c_master_setup(void) {
    ...
    I2C1BRG = 200;//1000; // I2CBRG = [1/(2*Fsck) - TPGD]*Pblck - 2 (TPGD is the Pulse Gobbler Delay)
...
void i2c_write(unsigned char addr, unsigned char cmd, unsigned char data){
    i2c_master_start();
...
unsigned char i2c_read(unsigned char write_addr, unsigned char read_addr, 
        unsigned char cmd, unsigned char ack){
    i2c_master_start();
    i2c_master_send(write_addr);
    i2c_master_send(cmd);
    i2c_master_restart();
    i2c_master_send(read_addr);
    unsigned char rtn_chr = i2c_master_recv();
    i2c_master_ack(ack);
    i2c_master_stop();
    return rtn_chr;
}